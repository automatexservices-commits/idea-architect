...
function BillingDashboardPage() {
  ...
  const upiId = billing?.summary.upiId ?? "7984390066@ptyes";
  const upiName = billing?.summary.upiName ?? "PLANNR";
  ...
  const payWithUpi = async () => {
    ...
    setError(null);
    try {
      const intent = await payViaUpiIntent("pro", {
        name: upiName,
        note: `PLANNR Pro plan for ${displayName}`,
      });
      setNotice(`Opening your UPI app with ${intent}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to open UPI payment.");
    }
  };
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            ...
            <aside className="rounded-[2rem] border border-border bg-white/80 p-5 shadow-[0_25px_70px_-50px_rgba(0,0,0,0.35)] sm:p-6 lg:p-7">
              ...
              <div className="mt-5 space-y-4">
                <div className="rounded-3xl border border-primary/30 bg-gradient-to-br from-primary/10 to-background p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="inline-flex items-center gap-3">
                      <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white shadow-sm ring-1 ring-border">
                        <img src={upiLogo} alt="UPI logo" className="h-8 w-auto object-contain" />
                      </div>
                      <div>
                        <div className="text-xs font-mono uppercase tracking-[0.24em] text-muted-foreground">UPI</div>
                        <div className="mt-1 text-lg font-semibold">India - Instant</div>
                      </div>
                    </div>
                    <div className="rounded-full border border-primary/20 bg-white px-3 py-1 text-[10px] font-mono uppercase tracking-[0.24em] text-muted-foreground">
                      Default
                    </div>
                  </div>

                  <div className="mt-4 rounded-2xl border border-border bg-white/70 p-4">
                    <div className="text-xl font-medium tracking-tight">{upiId}</div>
                    <div className="mt-1 text-sm text-muted-foreground">GPay - PhonePe - Paytm - BHIM</div>
                  </div>

                  <button
                    type="button"
                    onClick={payWithUpi}
                    className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground shadow-[0_6px_0_0_rgb(15_118_110)] transition-transform hover:translate-y-[1px]"
                  >
                    Pay via UPI app
                    <ArrowUpRight className="h-4 w-4" />
                  </button>
